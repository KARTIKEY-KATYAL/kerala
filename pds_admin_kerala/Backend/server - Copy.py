    ...
    if request.method == 'POST':
        try:
            ...
            if connection.is_connected():
                ...
                if user:
                    for row in user:
                        temp1 = {'State Name':'','WH_District': row[0], 'WH_Name': row[1], 'WH_ID': row[2], 'Type of WH': row[3], 'WH_Lat': row[4], 'WH_Long': row[5], 'Capacity': row[6], 'Processing':row[9], 'quantity of Wheat stored (Quintals)':''}
...
def fci_data():
    try:
        ...
        result = {'Warehouse_No': warehouse_no, 'FPS_No': fps_no, 'Total_Demand': total_demand,'Total_Demand_Rice': total_demand_rice, 'Total_Supply': total_supply, 'District_Count': districts_no,'Total_Demand_FRice': total_demand_frice}
...
def save_to_database(month, year, applicable):
    ...
    if connection.is_connected():
        ...
        create_fps_query = ("CREATE TABLE " + fps_table + " (district VARCHAR(100) NOT NULL, name VARCHAR(100) NOT NULL, id VARCHAR(100) NOT NULL, type VARCHAR(100) NOT NULL, Allocation_Wheat VARCHAR(100) NOT NULL,Allocation_Rice VARCHAR(100) NOT NULL,Allocation_FRice VARCHAR(100) NOT NULL,longitude VARCHAR(100) NOT NULL, latitude VARCHAR(100) NOT NULL, uniqueid VARCHAR(100) NOT NULL, active VARCHAR(10) NOT NULL DEFAULT '1')")
...
def save_to_database_leg1(month, year, applicable, scenario_type):
    ...
    if connection.is_connected():
        ...
        if(check_year_month_exists_leg1(connection, month, year)):
            ...
            sql = "UPDATE optimised_table_leg1 set applicable='" + applicable + "', last_updated='" + formatted_datetime + "', type='" + scenario_type + "' WHERE id='" + existingid + "'"; 
            #print(sql)
...
def processFile():
    ...
    if scenario_type == "intra":
        ...
        Total_Demand = []
        Total_Demand_FPS = {}
        Total_Demand = FPS['Allocation_Wheat'].sum() +FPS['Allocation_Rice'].sum()+ FPS['Allocation_FRice'].sum()
    ...
    #    Variables for Wheat from IG TO FPS
        ...
        for i in range(len(FPS["FPS_District"])):
            District_Name_FPS = FPS["FPS_District"][i]
            if District_Name_FPS not in District_Demand:
                District_Demand[District_Name_FPS] = float(FPS["Allocation_Wheat"][i]) + float(FPS["Allocation_Rice"][i])  + float(FPS["Allocation_FRice"][i])
            else:
                District_Demand[District_Name_FPS] += float(FPS["Allocation_Wheat"][i]) + float(FPS["Allocation_Rice"][i])+ float(FPS["Allocation_FRice"][i])
       ...
       # Calling CBC_CMB Solver
        ...
        df51.insert(2, 'From_State', 'Kerala')
        df51.insert(7, 'To', 'FPS')
        df51.insert(8, 'To_State', 'Kerala')
        ...
        df511.insert(2, 'From_State', 'Kerala')
        df511.insert(7, 'To', 'FPS')
        df511.insert(8, 'To_State', 'kerala')
        ...
        df5111.insert(1, 'From', 'Depot')
        df5111.insert(2, 'From_State', 'Kerala')
        df5111.insert(7, 'To', 'FPS')
        df5111.insert(8, 'To_State', 'Kerala')
        ...
        with pd.ExcelWriter('Backend//kerala_Distance_L2.xlsx') as writer:
        ...
        Cost = pd.ExcelFile("Backend//kerala_Distance_L2.xlsx")
...
def processFile_leg1():
    ...
    if scenario_type == "All Commodities from FCI":
       ...
       # Calling CBC_CMB Solver
        ...
        with pd.ExcelWriter('Backend//kerala_Distance_L1.xlsx') as writer:
            Distance3.to_excel(writer, sheet_name='BG_BG',index=False)
        ...
        Cost = pd.ExcelFile("Backend//kerala_Distance_L1.xlsx")
        BG_BG = pd.read_excel(Cost,sheet_name="BG_BG")
        ...
        data["WH_Used"] = df5['From_ID'].nunique()
        data["WH_Used_Baseline"] = "63"
        
        data["FPS_Used"] = df5['To_ID'].nunique()
        data["FPS_Used_Baseline"] = "288"
        
        data['Demand'] = float(WH['Allocation_Wheat'].sum()) + float(WH['Allocation_Rice'].sum()) +  float(WH['Allocation_FRice'].sum())
        data['Demand_Baseline'] = "9,46.338"
        
        data['Total_QKM'] = float(result)
        data['Total_QKM_Baseline'] = "52,928,857"
        
        data['Average_Distance'] = float(round(result, 2)) / Total_Demand
        data['Average_Distance_Baseline'] = "55.93"
        ...
        dist1=np.transpose(dist1)
        df31 = pd.DataFrame(data = dist1, index = node2['SW_ID'], columns = node3['WH_ID'])
        
        with pd.ExcelWriter('Backend//Distance_Matrix_Leg1.xlsx') as writer:
            df3.to_excel(writer, sheet_name='BG_BG')
            df31.to_excel(writer, sheet_name='BG_BG1')
        ...
        Total_Demand = []
        Total_Demand_FPS = {}
        Total_Demand = WH['Allocation_Wheat'].sum() +  WH['Allocation_Rice'].sum() + WH['Allocation_FRice'].sum() +  WH['Allocation_Wheat'].sum()
        Total_Demand_FPS['Total_Demand_Warehouse'] = Total_Demand  # Total demand
        ...
        for i in range(len(DCP['WH_ID'])):
            for j in range(len(WH['SW_ID'])):
                Variable7.append(str(DCP['WH_ID'][i]) + '_'
        ...
        DV_Variables7 = LpVariable.matrix('Y', Variable7, cat='float',
                lowBound=0)
        Allocation7 = np.array(DV_Variables7).reshape(len(DCP['WH_ID']),
                len(WH['SW_ID']))
        ...
        FCI_Warehouse1 = [[ PC_Mill1[j][i] for j in range(len( PC_Mill1))] for i in range(len( PC_Mill1[0]))]
        ...
        for i in range(len(FCI_Warehouse1)):
            for j in range(len(WH['SW_ID'])):
                allCombination7.append(Allocation7[i][j] * FCI_Warehouse1[i][j])        

        model += lpSum(allCombination4 + allCombination5 + allCombination6 +  allCombination7)
        ...
        for i in range(len(WH['SW_ID'])):
            model += ((lpSum(Allocation5[j][i] for j in range(len(FCI['WH_ID'
                           ])))) + (lpSum(Allocation7[j][i] for j in range(len(DCP['WH_ID'
                           ])))) >= WH['Allocation_Rice'][i])
        ...
        for i in range(len(DCP['WH_ID'])):
            model += ((lpSum(Allocation7[i][j] for j in range(len(WH['SW_ID'
                           ]))))  <= DCP['Processing'][i])

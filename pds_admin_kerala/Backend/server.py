...
def save_to_database_leg1(month, year, applicable):
    ...
    dcp_table = "dcp_leg1_" + str(random_id)
    if connection.is_connected():
        cursor = connection.cursor()
        current_datetime = datetime.now()
        formatted_datetime = current_datetime.strftime("%Y-%m-%d %H:%M:%S")
        if(check_year_month_exists_leg1(connection, month, year)):
            ...
            dcp_table = "dcp_leg1_" + str(existingid)
            cursor.execute(sql)
        else:
        ...
        connection.commit()
        copy_dcp_data = ("INSERT INTO " + dcp_table + " SELECT * FROM dcp WHERE active='1' AND type='mill'")
...
def processFile_leg1():
    ...
    stop_process = False
    scenario_type = request.form.get('type')
    if scenario_type == "all_Commodities_from_fci":
        try:
            USN = pd.ExcelFile('Backend//Data_2.xlsx')
            month = request.form.get('month')        
            year = request.form.get('year')

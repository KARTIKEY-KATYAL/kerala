...
<style>
    ...
    /* Optional: Font size for table header */
                ...
                <!-- START BREADCRUMB -->
                <ul class="breadcrumb">
                    <li><a href="MillView.php">Home</a></li>
    ...
    <!-- START SCRIPTS -->
		...
		document.getElementById('downloadCSV').addEventListener('click', async function() {
			try {
				...
				const csvResponse = await fetch('api/DownloadOptimalDataMill.php?format=csv&tableName1='+tableName1);
				const csvBlob = await csvResponse.blob();
				downloadFile(csvBlob, 'Mill_' + getDateString() + '.csv');
			} catch (error) {
		...
		document.getElementById('downloadXLSX').addEventListener('click', async function() {
			try {
				var tableName1 = '<?php echo $tablename1 ?>';
				const excelResponse = await fetch('api/DownloadOptimalDataMill.php?format=xlsx&tableName1='+tableName1);
				const excelBlob = await excelResponse.blob();
				downloadFile(excelBlob, 'Mill_' + getDateString() + '.xlsx');
			} catch (error) {
